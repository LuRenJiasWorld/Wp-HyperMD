...
class PrismJSAuto {
	...
	public function prism_styles_scripts() {
		...
		$prism_theme    = $this->get_option( 'highlight_library_style', 'hypermd_syntax_highlighting' ); //语法高亮风格
		$line_numbers   = $this->get_option( 'line_numbers', 'hypermd_syntax_highlighting' ) == 'on' ? true : false; //行号显示
		$show_language  = $this->get_option( 'show_language', 'hypermd_syntax_highlighting' ) == 'on' ? true : false; //显示语言
		$copy_clipboard = $this->get_option( 'copy_clipboard', 'hypermd_syntax_highlighting' ) == 'on' ? true : false; //粘贴
		...
		if ( empty( $prism_theme ) || $prism_theme == 'default' ) {
			...
			$prism_styles['prism-theme-style'] = $this->get_option( 'customize_my_style', 'hypermd_syntax_highlighting' ); //自定义风格
		} else {
